...
class MenuBar():
    ...
    def setupInfiniteSunButton(self):
        self.infinite_button_rect = pg.Rect(650, 8, 120, 32)
        self.infinite_button_font = getTextFont(18)

    def checkInfiniteSunButtonClick(self, mouse_pos):
        return self.infinite_button_rect.collidepoint(mouse_pos)

    def enableInfiniteSun(self):
        self.infinite_sun = True
        self.sun_value = c.INFINITE_SUN_VALUE
    ...
    def drawInfiniteSunButton(self, surface):
        if self.infinite_sun:
            bg_color = c.GOLD
            text_color = c.NAVYBLUE
            border_color = c.RED
        else:
            bg_color = c.LIGHTYELLOW
            text_color = c.NAVYBLUE
            border_color = c.NAVYBLUE
        pg.draw.rect(surface, bg_color, self.infinite_button_rect)
        pg.draw.rect(surface, border_color, self.infinite_button_rect, 2)
        text_image = self.infinite_button_font.render('无限太阳', True, text_color)
        text_rect = text_image.get_rect(center=self.infinite_button_rect.center)
    ...
    def draw(self, surface):
        ...
        for card in self.card_list:
            card.draw(surface)
        self.drawInfiniteSunButton(surface)
